...
async function check(directory) {
    ...
    const cryptoRequire = createRequire(projectRequire.resolve('eth-crypto'));
    const eccryptoRequire = createRequire(cryptoRequire.resolve('eccrypto'));
    const { ec: EC } = eccryptoRequire('elliptic');
    ...
    const transaction = await wallet.signTransaction({
        type: 0, to: '0x' + '5'.repeat(40), nonce: 0, gasLimit: 21000,
    ...
    console.log(`${target}: nonce regression and ethers 6 compatibility with ethers 5 signatures passed`);
}
