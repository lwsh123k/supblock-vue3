...
ethers 已升级到 6；EthCrypto → eccrypto 仍依赖 elliptic，因此保留补丁修复 [CVE-2025-14505](https://github.com/advisories/GHSA-848j-6mx2-7j84)。采用上游公开 [PR #345](https://github.com/indutny/elliptic/pull/345) 的相同逻辑；该 PR 尚未合并，本补丁不是上游发布的新版本。
...
安装后运行 `node scripts/check-elliptic-patch.cjs`，可检查实际安装包的两条 P-521 回归向量，以及 ethers 6 消息、交易签名是否与 ethers 5 的固定结果保持兼容；测试仅使用公开向量和合成密钥，不发送交易。
